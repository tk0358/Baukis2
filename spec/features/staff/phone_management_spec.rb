...
feature "職員による顧客電話番号管理" do
  ...
  scenario "職員が顧客の電話番号を追加する" do
    ...
    fill_in "form_customer_phones_0_number", with: "090-1111-2222"
    check "form_customer_phones_0_primary"
    click_button "更新"

    customer.reload
    expect(customer.personal_phones.size).to eq(1)
    expect(customer.personal_phones[0].number).to eq("090-1111-2222")
  end
  ...
  scenario "職員が顧客の自宅電話番号を追加する" do
    ...
    fill_in "form_home_address_phones_0_number", with: "0120-111-222"
    check "form_home_address_phones_0_primary"
    click_button "更新"

    customer.reload
    expect(customer.home_address.phones.size).to eq(1)
    expect(customer.home_address.phones[0].number).to eq("0120-111-222")
  end
  ...
  scenario "職員が顧客の勤務先電話番号を追加する" do
    ...
    fill_in "form_work_address_phones_0_number", with: "03-1111-2222"
    check "form_work_address_phones_0_primary"
    click_button "更新"

    customer.reload
    expect(customer.work_address.phones.size).to eq(1)
    expect(customer.work_address.phones[0].number).to eq("03-1111-2222")
