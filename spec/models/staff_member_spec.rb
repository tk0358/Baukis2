...
RSpec.describe StaffMember, type: :model do
  ...
  describe "値の正規化" do
    ...
    example "family_name_kanaに含まれるひらがなをカタカナに変換" do
      member = create(:staff_member, family_name_kana: "やまだ")
      expect(member.family_name_kana).to eq("ヤマダ")
    end

    example "family_name_kanaに含まれる半角カナを全角カナに変換" do
      member = create(:staff_member, family_name_kana: "ﾔﾏﾀﾞ")
      expect(member.family_name_kana).to eq("ヤマダ")
    end
  end
  ...
  describe "バリデーション" do
    ...
    example "漢字を含むfamily_name_kanaは無効" do
      member = build(:staff_member, family_name_kana: "山田")
      expect(member).not_to be_valid
    end

    example "長音符を含むfamily_name_kanaは有効" do
      member = build(:staff_member, family_name_kana: "クルーズ")
      expect(member).to be_valid
    end
